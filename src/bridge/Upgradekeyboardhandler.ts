...
import type { RelicDefinition }   from '../data/AllRelics';

// ---------------------------------------------------------------------------
// Types matching the bus payloads (kept local to avoid circular imports)
// ---------------------------------------------------------------------------

interface UpgradeAvailablePayload {
  upgrades:     UpgradeDefinition[];
  contextLabel: string;
  floor:        number;
}

interface RelicAvailablePayload {
...
export class UpgradeKeyboardHandler {
  ...
  static init(): void {
    if (UpgradeKeyboardHandler.initialised) return;
  ...
  private registerUpgradeShortcuts(): void {
    bus.on('upgrade:available', (e) => {
      const upgrades = (e.payload as UpgradeAvailablePayload).upgrades;
      ...
      const offSelected = bus.on('upgrade:selected', () => {
        cleanup(); offSelected(); offSkipped();
      });
  ...
  private registerRelicShortcuts(): void {
    bus.on('relic:available', (e) => {
      const relics = (e.payload as RelicAvailablePayload).relics;
  ...
  private registerMerchantShortcuts(): void {
