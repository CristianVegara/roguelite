...
export function openBuildInspector(run: RunResultDTO): void {
...
class BuildInspectorModal {
  ...
  constructor(private run: RunResultDTO) {
  ...
  private buildSectionA(): HTMLElement {
  ...
  private buildSectionB(): HTMLElement {
    const sec = el('div', 'bi-section');
    const lbl = el('div', 'bi-section-label'); lbl.textContent = 'BUILD';
    sec.appendChild(lbl);

    // Keystone
    if (this.run.keystone_owned) {
      const ks = el('div', 'bi-keystone');
      const ksPip = el('span', 'bi-ks-pip'); ksPip.textContent = '♛';
      const ksName = el('span', 'bi-ks-name'); ksName.textContent = this.run.keystone_owned;
  ...
  private buildSectionC(): HTMLElement {
    const sec = el('div', 'bi-section');
    const lbl = el('div', 'bi-section-label'); lbl.textContent = 'COMBAT STATS';
    sec.appendChild(lbl);
    ...
    stats.forEach(([label, value]) => {
      ...
      const lbl = el('span', 'bi-stat-lbl'); lbl.textContent = label;
      const val = el('span', 'bi-stat-val'); val.textContent = value;
      row.append(lbl, val);
