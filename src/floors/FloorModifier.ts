...
export type FloorModifierType =
  // ── Existing ─────────────────────────────────────────────────────────────
  | 'cursed'        // enemy stronger, extra gold on kill
  | 'elite'         // powerful enemy, 2× stats
  | 'frenzied'      // enemy attacks faster, deals less
  | 'fortified'     // enemy has double armor
  | 'darkened'      // crits deal no bonus damage
  | 'blessed'       // player deals +30% damage
  | 'thorned'       // thorns recoil on hit
  | 'volatile'      // enemy explodes on death
  | 'regenerating'  // enemy regenerates 1% max HP/sec
  | 'shielded'      // enemy starts with a shield
  // ── Phase 7: new combat modifiers ────────────────────────────────────────
  | 'mirrored'        // enemy reflects 20% of damage taken
  | 'vampiric_enemy'  // enemy heals 8% of damage dealt to player
  | 'enraged'         // enemy gains damage proportional to missing HP
  | 'glacial'         // enemy −50% speed, player −25% speed
  | 'sanctified'      // player heals 30% max HP before combat
  | 'storm'           // all attacks have +35% lightning chain chance
  | 'weakened'        // player armor = 0 vs enemy attacks
  | 'ancient'         // enemy has 200% HP, 50% damage
  | 'nightmare'       // player starts floor at 50% HP
  | 'hexed'           // player crit chance = 0
  | 'gilded'          // +30 gold on kill, enemy +30% HP
  | 'constricting'    // player current HP reduced by 25% at floor start
  // ── Phase 7: special floor types ─────────────────────────────────────────
  | 'treasure'      // no combat — bonus gold + free upgrade
  | 'merchant'      // no combat — spend gold at shop
  | 'chaos';        // meta: amplified random modifier

export interface FloorModifier {
  type:        FloorModifierType;
  name:        string;
  description: string;
  color:       number;
  /** Mutates enemy CombatStats before the fight begins. */
  mutateEnemy?: (stats: CombatStats) => void;
  /**
   * An engine flag string to set in RulesEngine.onFloorStart().
   */
  engineFlag?: FloorModifierType;
  /** Extra gold awarded on kill when this modifier is active. */
  bonusGold?: number;
  // ── Phase 7 additions ────────────────────────────────────────────────────
  /** If true, skip all combat on this floor. */
  skipCombat?: boolean;
  /** What kind of special floor this is (checked by GameScene). */
  specialType?: 'treasure' | 'merchant';
  /** Multiplier on player attack speed (0.75 = glacial slow). */
  playerSpeedMultiplier?: number;
  /** Bonus rewards for treasure floors. */
  bonusRewards?: {
    gold: number;
    freeUpgrade: boolean;
  };
...
export function buildSpecialEnemyStats(): CombatStats {
  return {
 ...
 */
export function rollFloorModifier(floor: number): FloorModifier | null {
  if (floor <= 2) return null;  // first two floors always normal
  ...
  if (Math.random() > combatChance) return null;
