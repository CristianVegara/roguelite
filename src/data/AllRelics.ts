      ...
      engine.registerTrigger('relic_soul_mirror', {
        ...
        action: (ctx) => {
          if (Math.random() < 0.30) {
            ...
            if (ctx.result.lightningDamage > 0) ctx.result.lightningDamage += ctx.result.lightningDamage;
            if (ctx.result.summonDamage > 0)    ctx.result.summonDamage   += ctx.result.summonDamage;
          }
        },
      });
    },
  ...
  },
  {
    ...
    onAcquire: (s, engine) => {
      ...
      engine.registerDefenseTrigger('relic_corruption_crystal', (ctx) => {
        const lives = data['revivesLeft'] as number;
  ...
  },
  {
    ...
    onAcquire: (s, engine) => {
      ...
      engine.registerDefenseTrigger('relic_battle_scar_tissue', (ctx) => {
        ...
        if (taken > 0) {
          data['damageTakenBank'] = (data['damageTakenBank'] as number) + taken;
          while ((data['damageTakenBank'] as number) >= 30) {
            data['damageTakenBank'] = (data['damageTakenBank'] as number) - 30;
            ctx.stats.maxHp += 1;
            ctx.stats.hp    += 1;
          }
        }
...
const LIFESTEAL: RelicDefinition[] = [
  {
...
const SCALING: RelicDefinition[] = [
  {
    ...
    onAcquire: (s, engine) => {
      ...
      engine.registerTrigger('relic_veterans_badge', {
        event: 'onFloorStart',
        action: (ctx) => {
          data['floorsCleared'] = (data['floorsCleared'] as number) + 1;
          if ((data['floorsCleared'] as number) % 10 === 0) {
...
interface RelicWeights { uncommon: number; rare: number; legendary: number; }

function relicWeightsForFloor(floor: number): RelicWeights {
  if (floor <= 5)  return { uncommon: 70, rare: 28, legendary: 2  };
  if (floor <= 15) return { uncommon: 55, rare: 35, legendary: 10 };
  if (floor <= 25) return { uncommon: 35, rare: 45, legendary: 20 };
  return              { uncommon: 20, rare: 45, legendary: 35 };
}

function weightedRelicRarity(w: RelicWeights): RelicRarity {
...
export type { RelicDefinition } from './RelicDefinition';
