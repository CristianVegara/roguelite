...
class StatsScreen {
  ...
  private buildContent(
    ...
    content.append(
      ...
      this.buildRunChart(runs),        // FIX: chart added between stats and milestones
   ...
   */
  private buildRunChart(runs: RunResultDTO[]): HTMLElement {
    const wrap = el('div', 'st-section');
  ...
  private buildMilestones(p: PlayerProfileDTO): HTMLElement {
    ...
    ALL_MILESTONES.forEach((m, i) => {
      ...
      if (i >= SHOW_MAX) chip.classList.add('is-hidden-milestone');
    ...
    if (ALL_MILESTONES.length > SHOW_MAX) {
      ...
      toggleBtn.addEventListener('click', () => {
        ...
        grid.querySelectorAll('.is-hidden-milestone').forEach(c => {
          (c as HTMLElement).style.display = expanded ? '' : 'none';
        });
