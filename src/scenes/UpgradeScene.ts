...
export interface UpgradeSceneData {
  ...
  floor:          number;
  /** Optional context label shown in the header (e.g. "Level 3" or "Boss Reward"). */
  context?:       string;
}
...
export class UpgradeScene extends Phaser.Scene {
  ...
  private createCard(upg: UpgradeDefinition, index: number): void {
    ...
    const stackElems: Phaser.GameObjects.GameObject[] = [];
    if (upg.maxStacks > 1) {
    ...
    // Flavour text
    const flavourElems: Phaser.GameObjects.GameObject[] = [];
    if (upg.flavour) {
  ...
  private selectUpgrade(upg: UpgradeDefinition): void {
