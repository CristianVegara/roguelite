...
export interface RelicSceneData {
...
export class RelicScene extends Phaser.Scene {
  ...
  private createCard(relic: RelicDefinition, cx: number): void {
    const cy  = CARD_Y;
    const hw  = CARD_W / 2;
    const hh  = CARD_H / 2;
    ...
    const flavElems: Phaser.GameObjects.GameObject[] = [];
    if (relic.flavour) {
