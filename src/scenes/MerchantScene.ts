...
import { pickRunUpgrades, UpgradeDefinition } from '../data/AllUpgrades';
...
export class MerchantScene extends Phaser.Scene {
  ...
  private rerollCost      = 15;   // increases each reroll
  ...
  private createRerollButton(): void {
    ...
    zone.on('pointerover', () => {
      if (this.engine.gold >= this.rerollCost) this.rerollBtnLabel.setColor('#ffffff');
    });
    ...
    zone.on('pointerdown', () => {
      ...
      this.children.getAll()
        .filter(c => (c as Phaser.GameObjects.GameObject & { name?: string }).name === 'merchant_card')
        .forEach(c => c.destroy());
      this.offers.forEach((offer, i) => this.createCard(offer, i));
  ...
  private createTempBuffSection(): void {
    const buffs: Array<{ label: string; desc: string; cost: number; apply: () => void }> = [
      {
        label: 'War Ration',
    ...
    buffs.forEach((buff, i) => {
      ...
      const drawBuff = (sold: boolean, hover = false): void => {
        bg.clear();
      ...
      zone.on('pointerover',  () => { if (!sold) drawBuff(false, true); });
  ...
  private createLeaveButton(): void {
    ...
    const drawDefault = (): void => {
      bg.clear(); bg.fillStyle(0x0e1428); bg.fillRoundedRect(cx - bw / 2, cy - bh / 2, bw, bh, 8);
      border.clear(); border.lineStyle(2, 0x4fc3f7); border.strokeRoundedRect(cx - bw / 2, cy - bh / 2, bw, bh, 8);
    };
    const drawHover = (): void => {
      bg.clear(); bg.fillStyle(0x162a50); bg.fillRoundedRect(cx - bw / 2, cy - bh / 2, bw, bh, 8);
      border.clear(); border.lineStyle(2, 0x7de8ff); border.strokeRoundedRect(cx - bw / 2, cy - bh / 2, bw, bh, 8);
    };
