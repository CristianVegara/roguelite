...
export class StatsScene extends Phaser.Scene {
  ...
  private buildContent(profile: PlayerProfileDTO | null, runs: RunResultDTO[]): void {
    const SCROLL_TOP = 52;
  ...
  private addPlayerCard(p: PlayerProfileDTO, y: number): number {
    const H = 68;
    ...
    this.content.add(
      this.add.text(38, y + H / 2, classIcon, {
    ...
    const titleStr = p.active_title
      ...
      : '';
    this.content.add(
      this.add.text(70, y + 14, `${titleStr}${p.name}`, {
    ...
    const streakStr = p.current_streak > 0
      ...
      : '';
    this.content.add(
      this.add.text(70, y + 33, `Joined ${joinDate}${streakStr}`, {
        fontSize: '10px', color: '#666688', fontFamily: 'monospace',
      }).setOrigin(0, 0),
    );

    this.content.add(
      this.add.text(70, y + 50, `${p.total_runs} runs  ·  ${formatTime(p.total_play_ms)} total play time`, {
  ...
  private addCareerStats(p: PlayerProfileDTO, y: number): number {
    ...
    for (let i = 0; i < stats.length; i++) {
      ...
      this.content.add(
        this.add.text(cx + CARD_W / 2, cy + 10, label, {
          fontSize: '8px', color: '#444466', fontFamily: 'monospace',
        }).setOrigin(0.5, 0),
      );
      this.content.add(
        this.add.text(cx + CARD_W / 2, cy + 24, value, {
  ...
  private addClassStats(p: PlayerProfileDTO, y: number): number {
    ...
    rows.forEach(([label, value]) => {
      ...
      this.content.add(
        this.add.text(16, y + 13, label, {
          fontSize: '10px', color: '#555577', fontFamily: 'monospace',
        }).setOrigin(0, 0.5),
      );
      this.content.add(
        this.add.text(GAME_WIDTH - 16, y + 13, value, {
  ...
  private addMilestones(p: PlayerProfileDTO, y: number): number {
    ...
    ALL_MILESTONES.forEach((m, i) => {
      ...
      this.content.add(
        this.add.text(cx + 8, cy + CHIP_H / 2, earned ? '✓' : '○', {
          fontSize: '10px', color: earned ? '#2ecc71' : '#333355', fontFamily: 'monospace',
        }).setOrigin(0, 0.5),
      );
      this.content.add(
        this.add.text(cx + 22, cy + CHIP_H / 2, m.title, {
  ...
  private addRecentRuns(runs: RunResultDTO[], y: number): number {
    this.content.add(this.sectionLabel('Recent Runs', y));
    y += 18;

    if (runs.length === 0) {
      this.content.add(
        this.add.text(GAME_WIDTH / 2, y + 10, 'No runs recorded yet.', {
    ...
    runs.forEach(run => {
      ...
      // Date
      this.content.add(
        this.add.text(16, y + ROW_H / 2, dateStr, {
          fontSize: '9px', color: '#444466', fontFamily: 'monospace',
        }).setOrigin(0, 0.5),
      );
      // Class
      this.content.add(
        this.add.text(72, y + ROW_H / 2, humaniseClass(run.class_id), {
          fontSize: '10px', color: '#9999bb', fontFamily: 'monospace',
        }).setOrigin(0, 0.5),
      );
      // Floor
      this.content.add(
        this.add.text(192, y + ROW_H / 2, `Fl. ${run.floor_reached}`, {
          fontSize: '11px', color: '#e0e0e0', fontFamily: 'monospace', fontStyle: 'bold',
        }).setOrigin(0, 0.5),
      );
      // Build
      this.content.add(
        this.add.text(252, y + ROW_H / 2, run.build_archetype, {
      ...
      // Duration
      this.content.add(
        this.add.text(GAME_WIDTH - 16, y + ROW_H / 2, dur, {
  ...
  private sectionLabel(text: string, y: number): Phaser.GameObjects.Text {
  ...
  private favoriteClassIcon(p: PlayerProfileDTO): string {
