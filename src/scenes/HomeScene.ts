...
const _CONTENT_H = GAME_HEIGHT - CONTENT_Y;  void _CONTENT_H;  // reserved for future scroll bounds
...
export class HomeScene extends Phaser.Scene {
  private tabContainers: Map<TabId, Phaser.GameObjects.Container> = new Map();
  private tabNavBtns: Phaser.GameObjects.Text[] = [];
  private tabNavUnderlines: Phaser.GameObjects.Graphics[] = [];

  // Refs that need live refresh
  private headerCurrencyText!: Phaser.GameObjects.Text;

  // Play tab — upgrade shop refs
  private upgradeRows: UpgradeRowRefs[] = [];
  private upgradeDrawerContainer!: Phaser.GameObjects.Container;
  private upgradeDrawerOpen = false;
  private upgradeDrawerBtnLabel!: Phaser.GameObjects.Text;
  ...
  private buildHeader(): void {
    ...
    this.add.text(GAME_WIDTH / 2, HEADER_H / 2, `${name}${streak}`, {
      ...
      .on('pointerout',  function(this: Phaser.GameObjects.Text) { this.setColor('#9999bb'); })
  ...
  private buildUpgradeDrawer(): void {
  ...
  private buildUpgradeRow(
    key: UpgradeKey, label: string, color: number, index: number,
  ): UpgradeRowRefs {
    const ROW_H  = 48;
    ...
    const refs: UpgradeRowRefs = { key, drawDots, levelT, btnBg, btnT, costT, btnX, rowY: y, rowH: ROW_H };
    this.upgradeDrawerContainer.add([rowBg, labelT, dotsG, levelT, btnBg, btnT, costT, zone]);
    this.refreshUpgradeRow(refs);
    return refs;
  }

  private refreshUpgradeRow(row: UpgradeRowRefs): void {
  ...
  private toggleUpgradeDrawer(): void {
  ...
  private buildLastRunPreview(floor: number, y: number): Phaser.GameObjects.Container {
  ...
  private buildProfileTab(): void {
    ...
    // Quick stats grid
    const quickStats: Array<[string, string]> = [
      ['WINS',    `${profile.wins}`],
  ...
  private confirmReset(): void {
    ...
    if ((this as unknown as Record<string, boolean>)['_resetPending']) {
      metaService.resetSave();
      ServiceLocator.profile.reset();
      ServiceLocator.history.clear();
      this.scene.start('NameEntryScene');
    } else {
      (this as unknown as Record<string, boolean>)['_resetPending'] = true;
      this.time.delayedCall(3000, () => {
        (this as unknown as Record<string, boolean>)['_resetPending'] = false;
      });
    }
  }
  ...
  private getBestRunForMode(modeId: string): RunResultDTO | null {
  ...
  private getBestRunsPerMode(runs: RunResultDTO[]): Array<{ mode: GameModeConfig; run: RunResultDTO }> {
    const best = new Map<string, RunResultDTO>();
    runs.forEach(r => {
  ...
  private makeRowText(
    x: number, y: number, text: string, color: string, bold = false,
...
// ---------------------------------------------------------------------------
// Upgrade drawer row refs
// ---------------------------------------------------------------------------

interface UpgradeRowRefs {
  key:      UpgradeKey;
  drawDots: (level: number) => void;
  levelT:   Phaser.GameObjects.Text;
  btnBg:    Phaser.GameObjects.Graphics;
  btnT:     Phaser.GameObjects.Text;
  costT:    Phaser.GameObjects.Text;
  btnX:     number;
  rowY:     number;
  rowH:     number;
}
