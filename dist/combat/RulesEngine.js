...
export class RulesEngine {
    ...
    get totalDamageTaken() { return this.state.totalDamageTaken; }
    /** Current charge progress for ball_lightning / spark (resets to 0 after proc). */
    get chargeCounter() { return this.state.chargeCounter; }
    ...
    storeOverkill(overkill) {
        if (this.hasUpgrade('overflow') && overkill > 0) {
            this.state.overflowStored = Math.min(Math.floor(overkill * 0.5), 1000000000);
        }
    }
    /**
