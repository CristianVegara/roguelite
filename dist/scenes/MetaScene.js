...
export class MetaScene extends Phaser.Scene {
    ...
    // ---------------------------------------------------------------------------
    buildUpgradeSection() {
        this.rowRefs = UPGRADE_INFO.map((info, i) => this.buildUpgradeRow(info.key, info.label, info.bonusDesc, info.color, ROW_Y[i]));
        this.refreshDynamicUI();
    }
    buildUpgradeRow(key, label, bonusDesc, color, cy) {
        const hh = ROW_H / 2;
        // Row background
