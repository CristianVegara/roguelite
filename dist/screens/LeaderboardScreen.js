...
class LeaderboardScreen {
    ...
    build() {
        ...
        root.append(this.buildHeader(), this.buildFilterBar(), this.buildTable(), this.buildCardList());
        return root;
    }
    // ── Header ──────────────────────────────────────────────────────────────────
    buildHeader() {
    ...
    buildFilterBar() {
        ...
        const modeSelect = this.buildSelect('Mode', [{ value: '', label: 'All Modes' }, ...MODES_REGISTRY.map(m => ({ value: m.id, label: m.name }))], this.filters.modeId, (v) => { this.filters.modeId = v; this.saveFilters(); this.refresh(); });
        const allClasses = [...new Set(this.allRuns.map(r => r.class_id))].sort();
        const classSelect = this.buildSelect('Class', [{ value: '', label: 'All Classes' },
            ...allClasses.map(c => ({ value: c, label: humanise(c) }))], this.filters.classId, (v) => { this.filters.classId = v; this.saveFilters(); this.refresh(); });
        const sortSelect = this.buildSelect('Sort', [
            { value: 'floor', label: 'Floor (desc)' },
            { value: 'score', label: 'Score (desc)' },
            { value: 'kills', label: 'Kills (desc)' },
            { value: 'date', label: 'Date (newest)' },
        ], this.filters.sort, (v) => { this.filters.sort = v; this.saveFilters(); this.refresh(); });
        bar.append(modeSelect, classSelect, sortSelect);
        return bar;
    }
    buildSelect(label, options, current, onChange) {
    ...
    refresh() {
        ...
        if (this.filters.modeId)
            runs = runs.filter(r => r.mode_id === this.filters.modeId);
        if (this.filters.classId)
            runs = runs.filter(r => r.class_id === this.filters.classId);
        const sort = this.filters.sort;
        runs.sort((a, b) => {
            if (sort === 'floor')
                return b.floor_reached - a.floor_reached;
            if (sort === 'score')
    ...
    refreshTable(runs) {
        ...
        runs.forEach((run, i) => {
            ...
            const date = el('span', 'lb-date');
            date.textContent = dateStr;
            const cls = el('span', 'lb-cls');
            cls.textContent = humanise(run.class_id);
            const mode = el('span', 'lb-mode');
            mode.textContent = modeCfg ? modeCfg.icon + ' ' + modeCfg.name : run.mode_id;
            const floor = el('span', 'lb-floor');
            floor.textContent = String(run.floor_reached);
            const build = el('span', 'lb-build');
            build.textContent = run.build_archetype;
            const kills = el('span', 'lb-kills');
            kills.textContent = String(run.kills);
