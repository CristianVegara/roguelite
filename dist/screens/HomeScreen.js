...
class HomeScreen {
    ...
    build() {
        ...
        root.append(this.buildHeader(), this.buildTabNav(), this.buildTabContent('play', () => this.buildPlayPane()), this.buildTabContent('upgrades', () => this.buildUpgradesPane()), this.buildTabContent('history', () => this.buildHistoryPane()), this.buildTabContent('profile', () => this.buildProfilePane()), this.buildTabContent('settings', () => this.buildSettingsPane()));
        return root;
    }
    // ── Header ─────────────────────────────────────────────────────────────────
    buildHeader() {
    ...
    buildTabNav() {
        ...
        const tabs = ['play', 'upgrades', 'history', 'profile', 'settings'];
    ...
    buildPlayPane() {
        ...
        if (lastRun) {
            ...
            const wt = el('div', 'hs-welcome-title');
            wt.textContent = 'Welcome to The Spire';
    ...
    buildModeCard(mode) {
        ...
        const iconEl = el('span', 'hs-mode-icon');
        iconEl.textContent = mode.icon;
        const nameEl = el('span', 'hs-mode-name');
        nameEl.textContent = mode.name.toUpperCase();
    ...
    buildUpgradeRow(key, label, color) {
    ...
    buildHistoryPane() {
        ...
        if (bestByMode.length === 0) {
            ...
            const emIcon = el('div', 'hs-history-empty-icon');
            emIcon.textContent = '\u{1F3AF}';
            ...
            bestByMode.forEach(({ mode, run }) => {
                ...
                const modeEl = el('span', 'hs-ranks-mode');
                modeEl.textContent = mode.icon + '  ' + mode.name;
                const statEl = el('span', 'hs-ranks-stat');
                statEl.textContent = statVal;
                const buildEl = el('span', 'hs-ranks-build');
                buildEl.textContent = run.build_archetype;
                const dateEl = el('span', 'hs-ranks-date');
    ...
    buildProfilePane() {
        ...
        if (bestRun) {
            ...
            const blabel = el('span', 'hs-best-label');
            blabel.textContent = 'PERSONAL BEST';
            const bfloor = el('span', 'hs-best-floor');
            bfloor.textContent = 'Floor ' + bestRun.floor_reached;
        ...
        const nameEl = el('div', 'hs-profile-name');
        ...
        stats.forEach(([label, val]) => {
            ...
            const lbl = el('span', 'hs-stat-lbl');
            lbl.textContent = label;
            const valEl = el('span', 'hs-stat-val');
        ...
        if (profile.unlocked_titles.length === 0) {
            ...
            profile.unlocked_titles.forEach(id => {
                ...
                if (isActive) {
                    ...
                    const wearing = el('span', 'hs-title-wearing');
                    wearing.textContent = 'wearing';
                    chip.appendChild(wearing);
                }
                chip.addEventListener('click', () => {
                    if (id === profile.active_title)
                        return;
                    ServiceLocator.profile.updateProfile({ active_title: id });
                    profile.active_title = id;
                    titleEl.textContent = '[' + id + ']';
                    if (!card.contains(titleEl))
                        card.appendChild(titleEl);
                    ...
                    chip.classList.add('is-active');
                    const w = el('span', 'hs-title-wearing');
                    w.textContent = 'wearing';
                    chip.appendChild(w);
                });
    ...
    buildSettingsPane() {
        ...
        const renameRow = this.buildSettingsRow('Change Name', 'Opens name entry to change your display name', 'RENAME \u2192', () => {
            sessionStorage.setItem('nameEntryContext', 'rename');
            router.navigate('name-entry');
        });
        pane.appendChild(renameRow);
        let resetPending = false;
        const resetRow = this.buildSettingsRow('Reset All Data', 'Clears all progress, currency and statistics', 'RESET \u2192', (btn) => {
            if (resetPending) {
        ...
        binds.forEach(([key, desc]) => {
            ...
            const keyEl = el('span', 'hs-bind-key');
            keyEl.textContent = '[' + key + ']';
            const descEl = el('span', 'hs-bind-desc');
    ...
    buildSettingsRow(title, desc, btnLabel, onClick, danger = false) {
        const row = el('div', 'hs-settings-row');
        const info = el('div', 'hs-settings-info');
        const titleEl = el('div', 'hs-settings-title');
        titleEl.textContent = title;
        if (danger)
            titleEl.classList.add('is-danger');
        const descEl = el('div', 'hs-settings-desc');
        descEl.textContent = desc;
