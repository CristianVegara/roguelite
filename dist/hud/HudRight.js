...
export class HudRight {
    ...
    // ── Subscriptions ─────────────────────────────────────────────────────────
    subscribe() {
        this.subs.push(runState.subscribe(s => `${s.enemyHp}|${s.enemyMaxHp}|${s.enemyName}|${s.isBoss}`, () => {
            const s = runState.get();
            this.updateEnemy(s.enemyHp, s.enemyMaxHp, s.enemyName, s.isBoss);
        }));
        this.subs.push(runState.subscribe(s => s.enemyPoisonStacks, (stacks) => {
            if (stacks > 0) {
        ...
        this.subs.push(runState.subscribe(s => s.enemyBurnStacks, (stacks) => {
            if (stacks > 0) {
    ...
    rebuildSummonUpgradeList(upgrades) {
