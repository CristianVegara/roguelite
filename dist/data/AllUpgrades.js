    ...
    },
    {
        ...
        apply: (s, e) => {
            ...
            e.registerDefenseTrigger('hp_up_ongoing', (ctx) => {
                if (ctx.result.damageTaken <= 0)
                    return;
                const d = ctx.engine.getRelicData('hp_up');
                const prev = d['accumulator'] ?? 0;
            ...
            e.registerTrigger('perpetual_machine', {
                ...
                action: (ctx) => {
                    if (ctx.engine.hitStreak > 0 && ctx.engine.hitStreak % 10 === 0) {
                        ...
                        if ((ctx.stats.poisonChance ?? 0) > 0)
                            ctx.result.poisonApplied = Math.max(ctx.result.poisonApplied, ctx.stats.poisonStacks ?? 1);
                        if ((ctx.stats.burnChance ?? 0) > 0)
                            ctx.result.burnApplied = true;
                        if ((ctx.stats.lightningChance ?? 0) > 0)
                            ctx.result.lightningDamage = Math.max(ctx.result.lightningDamage, Math.floor(ctx.damage * (ctx.stats.lightningDamage ?? 0.5)));
                    }
                },
            });
        },
...
function rarityWeightsForFloor(floor) {
    if (floor <= 3)
        return { common: 80, uncommon: 18, rare: 2, legendary: 0 };
    if (floor <= 6)
        return { common: 55, uncommon: 30, rare: 13, legendary: 2 };
    if (floor <= 10)
        return { common: 35, uncommon: 32, rare: 24, legendary: 9 };
    if (floor <= 15)
        return { common: 15, uncommon: 30, rare: 35, legendary: 20 };
...
function weightedRarityRoll(weights) {
    ...
    if ((roll -= weights.common) < 0)
        return 'common';
    if ((roll -= weights.uncommon) < 0)
        return 'uncommon';
    if ((roll -= weights.rare) < 0)
        return 'rare';
    return 'legendary';
}
// ---------------------------------------------------------------------------
 ...
 */
export function pickRunUpgrades(count, floor, owned, classWeights, guaranteeCategory) {
    ...
    const buildWeightedPool = (source) => {
        if (!classWeights)
            return source;
        const pool = [];
    ...
    if (guaranteeCategory) {
        const catPool = regular.filter(u => u.category === guaranteeCategory && isAvailable(u));
        if (catPool.length > 0) {
    ...
    for (let i = 0; i < count; i++) {
        if (i === keystoneSlot && chosen.length >= 1)
            continue; // already filled
        const rarity = weightedRarityRoll(weights);
        const weightedRegular = buildWeightedPool(regular);
        const pool = weightedRegular.filter(u => u.rarity === rarity && isAvailable(u) && !usedIds.has(u.id));
    ...
    if (floor >= 10) {
        ...
        if (!hasAdvanced) {
            const advPool = regular.filter(u => u.tier !== 'starter' && isAvailable(u) && !usedIds.has(u.id));
